...
if os.name == 'nt':
    ...
    # bert_path = './chinese_L-12_H-768_A-12/'
    bert_path = './our_model/'
    root_path = './bert-multiclass-master'
...
flags.DEFINE_bool(
    "do_retrain", False,
    "Whether to train. Should be False for pretrain "
    "models and False for cased models.")
...
# flags.DEFINE_bool("do_retrain", False, "Whether to run training.")
...
class MultilabelClassfier(DataProcessor):
    ...
    def get_train_examples(self, data_dir):
        ...
        return self._create_examples(
            self._read_tsv(os.path.join(data_dir, FLAGS.train)), "train")

    def get_dev_examples(self, data_dir):
        """See base class."""
        return self._create_examples(
            self._read_tsv(os.path.join(data_dir, FLAGS.dev)), "dev")

    def get_test_examples(self, data_dir):
        """See base class."""
    
        return self._create_examples(
            self._read_tsv(os.path.join(data_dir, FLAGS.test)), "test") #all_nolabel_data.json
...
def generate_pred_file(tags_list, inf_path, outf_path):
    with open(inf_path, 'r',encoding='utf-8') as inf,open(outf_path, 'w', encoding='utf-8') as outf:
        pred_labels,result_pro = result_labels()
        sum = 0
        count = 0
        for line in inf.readlines():
            ...
            for ind in range(len(pre_doc)):
                ...
                pred_sent['pro'] = result_pro[sum].tolist()
...
if __name__ == "__main__":
    ...
    #inf_bool = input("get data for abl（y/n）：")
    print("=================================yeap======================================")
    print(FLAGS.do_train)

    # inf_path = 'data/all_data/10_new.json'
    # outf_path = 'tmp/newoutput/output_22453_10_new.json'
    inf_path = 'data/all_data/40.json'
    outf_path = 'tmp/newoutput/output_nolabel_new.json'

    if FLAGS.do_retrain == True:
        FLAGS.do_train = True
        FLAGS.do_eval = True
        FLAGS.train = "retrain.json"
        FLAGS.test = "10_new.json"
        FLAGS.num_train_epochs = 3 
        inf_path = 'data/all_data/10_new.json'
        outf_path = 'tmp/newoutput/output_22453_10_new_abl.json'

    with open('data/all_data/tags.txt', 'r', encoding='utf-8') as tagf:
      for line in tagf.readlines():
          tags_list.append(line.strip())
    generate_pred_file(tags_list, inf_path, outf_path)
    print(FLAGS.do_train)
